			...
			using (var connection = Gravitybox.Datastore.EFDAL.DBHelper.GetConnection(Gravitybox.Datastore.EFDAL.Util.StripEFCS2Normal(connectionString)))
			{
				using (var dc = new DataContext(connection))
				{
					...
					using (var cmd = BusinessEntityQuery.GetCommand<Gravitybox.Datastore.EFDAL.ServerQuery>(dc, template, where))
					{
						...
						var sql = "CREATE TABLE #t ([ServerId] [Int])";
						sql += "set rowcount " + optimizer.ChunkSize + ";";
						sql += "INSERT INTO #t ([ServerId])";
						sql += "SELECT [t0].[ServerId] #t\r\n";
						...
						sql += "DELETE [Server] FROM [dbo].[Server] " + noLock + "INNER JOIN #t ON [dbo].[Server].[ServerId] = #t.[ServerId]\r\n";
						sql += ";select @@rowcount";
