...
namespace Gravitybox.Datastore.Server.Core
{
    internal static class SqlHelper
    {
        ...
        internal static DataSet GetDataset(string connectionString, string sql, List<SqlParameter> parameters = null, int timeOut = 60, int maxRetry = 0)
        {
        ...
        public static bool AddRepository(string connectionString, RepositorySchema fullSchema, string fileGroup)
        {
            try
            {
                ...
                if (hasDataGrouping)
                {
                    ...
                    LoggerCQ.LogInfo($"DataGrouping Add: ID={fullSchema.ID}, Fields=\"{string.Join("|", dgList.Select(z => z.Name))}\"");
                    ...
                    sb.AppendLine($"CREATE CLUSTERED INDEX [{indexName}] ON [{dataTable}] ({pkFields.ToCommaList()}) WITH (FILLFACTOR=80);");
                    sb.AppendLine();
                }
                #endregion
                ...
                //Add Hash index
                {
                    ...
                    sb.AppendLine($"CREATE NONCLUSTERED INDEX [{indexName}] ON [{dataTable}] ([{SqlHelper.HashField}] ASC) WITH (FILLFACTOR=80){GetSqlIndexFileGrouping()};");
        ...
        internal static string GetRepositorySql(RepositorySchema schema, List<string> indexList = null)
        {
            ...
            foreach (var field in schema.FieldList)
            {
                ...
                if (useIndex)
                {
                    ...
                    sb.Append($"CREATE {pkAttr}NONCLUSTERED INDEX [{indexName}] ON [{dataTable}] ([{field.TokenName}] {(field.SearchAsc ? "ASC" : "DESC")}) WITH (FILLFACTOR=80){GetSqlIndexFileGrouping()}");
                    sb.AppendLine(";");
                ...
                x.DataType != RepositorySchema.DataTypeConstants.GeoCode).ToList())
            {
                ...
                sb.AppendLine("BEGIN");
                sb.Append($"CREATE NONCLUSTERED INDEX [{indexName}] ON [{dataTable}] ([{dimensionColumnName}]) WITH (FILLFACTOR=80){GetSqlIndexFileGrouping()};");
        ...
        public static DataQueryResults Query(RepositorySchema schema, int repositoryId, DataQuery query, List<DimensionItem> dimensionList, out string executeHistory, int extraRecords = 0)
        {
            try
            {
                ...
                if (query.IncludeRecords)
                    DataManager.Sync(configuration.retval.RecordList.ToList(), schema);
            ...
            try
            {
                ...
                if (query.DimensionValueList != null && query.DimensionValueList.Any())
                {
                    ...
                    foreach (var dItem in prepList.Keys)
                    {
                        ...
                        {
                            LoggerCQ.LogWarning($"GetWhereClause could not find dimension {dItem.Name}|{dItem.DIdx}");
                        }
                    }
                }
                ...
                else if (field.DataType == RepositorySchema.DataTypeConstants.String)
                {
                    ...
                    switch (ff.Comparer)
                    {
                        ...
                        case ComparisonConstants.Like:
                            {
                                var frontMatch = ffValue?.StartsWith("*") == true ? "'%'" : "''";
                                ...
                                parameters.Add(filterParam);
                                sb.Append($"[Z].[{field.TokenName}] LIKE {frontMatch}+{filterParam.ParameterName}+{endMatch}");
                            }
                            break;
                        case ComparisonConstants.ContainsAny:
                            if (ffValue != null)
        ...
        internal static int Count(RepositorySchema schema, string connectionString)
        {
            try
            {
                var dataTable = GetTableName(schema);
                var ds = GetDataset(connectionString, $"select count(*) from [{dataTable}] {NoLockText()}");
                if (ds.Tables.Count == 1 && ds.Tables[0].Rows.Count == 1)
        ...
        internal static int Clear(RepositorySchema schema, string connectionString)
        {
            try
            {
               RepositorySchema parentSchema = null;
                ...
                var count = Count(schema, connectionString);
        ...
        public static SqlResults DeleteData(RepositorySchema schema, DataQuery query, List<DimensionItem> dimensionList)
        {
            ...
            do
            {
                try
                {
                    ...
                    #region Mark these records for later delete out of dependent tables
                    long deleteId = 0;
                    using (var context = new DatastoreEntities())
                    {
                        //Create a new delete entry
                        var newEntry = new EFDAL.Entity.DeleteQueue { RepositoryId = schema.InternalID, IsReady = false };
                        context.AddItem(newEntry);
                        context.SaveChanges();
                    ...
                    //Mark the delete queue item as ready for processing
                    using (var context = new DatastoreEntities())
                    {
                        context.DeleteQueue.Where(x => x.RowId == deleteId).Update(x => new EFDAL.Entity.DeleteQueue { IsReady = true });
                        context.SaveChanges();
                    }
                    ...
                    foreach (var id in repositoryKeyList)
                    {
                        ...
                        catch (SqlException ex)
                        {
                            if (ex.ToString().Contains("Timeout Expired"))
                                LoggerCQ.LogWarning($"DefragFTS Failed: ID={id}, Error=TimeoutExpired");
                            else
                                LoggerCQ.LogWarning($"DefragFTS Failed: ID={id}, Error={ex.Message}");
                        }
        ...
        public static bool DeleteDimensionValue(RepositorySchema schema, List<DimensionItem> dimensionList, long dvidx)
        {
            ...
            if (dDef != null && dDef.DimensionType == RepositorySchema.DimensionTypeConstants.Normal)
            {
                ...
                var dataTable = GetTableName(schema);
                sb.AppendLine($"update [{dataTable}] set [{dDef.TokenName}] = NULL, [__d{dDef.TokenName}] = NULL where [__d{dDef.TokenName}] = @dvidx");
            }
        ...
        private static string GetListTableCreate(string dimensionTable)
        {
            ...
            sb.AppendLine($"CONSTRAINT [PK_{dimensionTable}] PRIMARY KEY CLUSTERED ([DVIdx], [{RecordIdxField}]) WITH (FILLFACTOR=80)");
            sb.AppendLine(")");
            if (!string.IsNullOrEmpty(fileGroup)) sb.AppendLine($"ON [{fileGroup}];");
            else sb.AppendLine(";");

            //Create covering index
            var indexName = $"NCK_{dimensionTable}";
            sb.AppendLine($"if not exists(select * from sys.indexes where name = '{indexName}')");
            sb.AppendLine($"CREATE NONCLUSTERED INDEX [{indexName}] ON [{dimensionTable}] ([{SqlHelper.RecordIdxField}], [DVIdx]) WITH (FILLFACTOR=80)");
            if (!string.IsNullOrEmpty(fileGroup)) sb.AppendLine($"ON [{fileGroup}];");
            else sb.AppendLine(";");
            ...
            if (ConfigHelper.SupportsCompression)
            {
                ...
                sb.AppendLine($"ALTER INDEX [PK_{dimensionTable}] ON [{dimensionTable}] REBUILD WITH (DATA_COMPRESSION = PAGE);");
                sb.AppendLine($"ALTER INDEX [{indexName}] ON [{dimensionTable}] REBUILD WITH (DATA_COMPRESSION = PAGE);");
            }
        ...
        private static string GetUserPermissionTableCreate(RepositorySchema schema)
        {
            ...
            sb.Append($"CONSTRAINT [PK_{userPermissionTableName.ToUpper()}] PRIMARY KEY CLUSTERED ([UserId], [FKField]) WITH (FILLFACTOR=80){GetSqlIndexFileGrouping()})");
            if (!string.IsNullOrEmpty(fileGroup)) sb.AppendLine($"ON [{fileGroup}];");
            else sb.AppendLine(";");

            //Create covering index
            var indexName = $"NCK_{userPermissionTableName}";
            sb.AppendLine($"if not exists(select * from sys.indexes where name = '{indexName}')");
            sb.Append($"CREATE NONCLUSTERED INDEX [{indexName}] ON [{userPermissionTableName}] ([FKField], [UserId]) WITH (FILLFACTOR=80)");
            if (!string.IsNullOrEmpty(fileGroup)) sb.AppendLine($"ON [{fileGroup}];");
            else sb.AppendLine(";");
        ...
        public static DataQueryResults DimensionDump(DataQuery query, RepositorySchema schema)
        {
            try
            {
                var timer = Stopwatch.StartNew();
                ...
                retval.DimensionList = schema.DimensionList
                    .Select(x => new DimensionItem
                    {
                        DIdx = x.DIdx,
                        Name = x.Name,
                        NumericBreak = x.NumericBreak,
                        RefinementList = new List<IRefinementItem>(),
                    })
                    .ToList();
                ...
                //Order refinements
                retval.DimensionList.ForEach(x => x.RefinementList = x.RefinementList.OrderBy(z => z.FieldValue).ToList());
        ...
        public static bool IsFTSReady(Guid repositoryId)
        {
            ...
            {
                LoggerCQ.LogError($"IsFTSReady Error: {ex.Message}");
            }
            return false;
        }

        private static bool IsContainsComparer(ComparisonConstants value)
        {
            return value == ComparisonConstants.ContainsAll ||
                value == ComparisonConstants.ContainsAny ||
                value == ComparisonConstants.ContainsNone;
        }
